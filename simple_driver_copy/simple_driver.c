...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
	...
	list_for_each(p, &message_list)
	{
		phl = list_entry(p, high_level_t, link);
		printk("f = %d\n", phl->f);
	} /* End for list */
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
	struct element *el;
	el = kzalloc(sizeof(struct element), GFP_KERNEL);
	el->msg = kmalloc(strlen(buffer)+1, GFP_KERNEL);
	sprintf(el->msg, "%s(%zu letters)", buffer, len);   // appending received string with its length
	list_add_tail(&el->node, &message_list);
	size_of_message = strlen(el->msg);                 // store the length of the stored message
